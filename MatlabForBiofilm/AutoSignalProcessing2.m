...
for j=1:1:8735
    ...
    folderName='/Users/mikamihiroshi/Dropbox/Programs/Biofilm/MembranePotentialModel/OscillationModel/Oscillation_2D/Hex/FullPaper/Revised/アーカイブ/AutoModes0623_revised1/1/Results/ParameterSet';
    ...
    if FlagSingle==0
        ...
        interval=XData(2)-XData(1);
        StepStart=cast((XlimStart+interval-min(XData))/interval,'int64');
        StepEnd=cast((XlimEnd+interval-min(XData))/interval,'int64');
        XData=XData(StepStart:StepEnd);
        [pks1,TF1] = findpeaks(dDataLeft(StepStart:StepEnd),'MinPeakProminence',0.05);
        %[pks1,TF1] = findpeaks(FilterdDataLeft,'MinPeakProminence',0.05e+0);
        plot(XData(TF1),pks1,'r*','LineWidth',LineHaba);%極小値をグラフに追加
        [pks2,TF2] = findpeaks(dDataRight(StepStart:StepEnd),'MinPeakProminence',0.05);
        % [pks2,TF2] = findpeaks(FilterdDataRight,'MinPeakProminence',0.05e+0);
        plot(XData(TF2),pks2,'g*','LineWidth',LineHaba);%極小値をグラフに追
        xlim([XlimStart XlimEnd]);
        %}
        ...
        [pks1,TF1] = findpeaks(dDataLeft(StepStart:StepEnd),'MinPeakProminence',0.05);
        %[pks1,TF1] = findpeaks(FilterdDataLeft,'MinPeakProminence',0.05e+0);
        plot(XdData(TF1),pks1,'r*','LineWidth',LineHaba);%極小値をグラフに追加
        [pks2,TF2] = findpeaks(dDataRight(StepStart:StepEnd),'MinPeakProminence',0.05);
        % [pks2,TF2] = findpeaks(FilterdDataRight,'MinPeakProminence',0.05e+0);
        plot(XdData(TF2),pks2,'g*','LineWidth',LineHaba);%極小値をグラフに追
        xlim([XlimStart XlimEnd]);
