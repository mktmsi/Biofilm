...
folderName='/Users/mikamihiroshi/Dropbox/Programs/Biofilm/MembranePotentialModel/OscillationModel/Oscillation_2D/Hex/FullPaper/AutoModes/Results0609/ParameterSet';
...
if FlagSingle==0
    ...
    figure;
    %%左側のバイオフィルム
    XDataBibun=XData(1:row2)
    ax=plot(XDataBibun,dDataLeft,'LineWidth',LineHaba);
    %findpeaks(FilterdDataLeft,XDataFiltered)
    hold on;
    %%右側のバイオフィルム をプロット
    plot(XDataBibun,dDataRight,'-.','LineWidth',LineHaba);
    c=legend('Biofilm1','Biofilm2');%凡例を制御
    c.FontSize=HanreiSize;
    c.Location='northwest';
    set(gca,'FontSize',MemoriSize);
    ...
    ylabel('rate Not Filtered ','FontSize',LabelFontSize,'FontWeight','bold')
    axes('position',ax.Position,'box','on','ytick',[],'xtick',[],'color','none','LineWidth',LineHaba)%メモリのない上側のx軸と右側のy軸を追加
    ...
    %{
    %%各グラフの極小点を検出し，グラフに追加
    TF1 = islocalmin(FilterdDataLeft, 'FlatSelection', 'first','MinProminence',0.05)%極小値検出．起伏が0.05以上の点のみ抽出
    plot(XDataFiltered(TF1),FilterdDataLeft(TF1),'r*','LineWidth',LineHaba);%極小値をグラフに追加
    TF2 = islocalmin(FilterdDataRight, 'FlatSelection', 'first','MinProminence',0.05)%極小値検出．起伏が0.05以上の点のみ抽出
   ...
   % [pks2,TF2] = findpeaks(FilterdDataRight,'MinPeakProminence',0.05e+0);
    ...
    if Num<2
        ...
        for i=1:Num-1
            ...
            PhiDiff(i)=2*pi*(((k(i+1)-k1(i))/Bunbo1(i))-((k(i+1)-k2(i))/Bunbo2(i)));
            if abs(PhiDiff(i))>pi
                PhiDiff(i)=2.0*pi-abs(PhiDiff(i));
            end
        end
        k
    end
    PhiDiff/pi
    Results=mean(rem(abs(PhiDiff),2*pi))/pi
    %{
    if abs(Results)>1.0
        Results=2.0-abs(Results)
    end
    %}
end
